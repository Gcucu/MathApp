...
input.addEventListener('keypress', function func() {
	input.removeEventListener('keypress', func);
});
...
input.addEventListener('keypress', function(event) {
	if (event.code == 'Enter'){
		...
		ecran.innerHTML = '';
		if ((n > 0) && (n <= 10)){
		p0.innerHTML = `Tabla adunării cu ${n}:`;
		elem.appendChild(p0); 
		operatie();
		}		
		input.value = null;
...
function afisez(nr, num){
	...
	for (let i = 0; i < ((Number(nr) + Number(num))/10); i++){
		let tr1 = document.createElement('tr');
			for (let j = 0; j < 10; j++){
				...
				if (k < Number(nr)){
					...
					k++;	
				} else if ((k >= Number(nr)) && (k < (Number(num) + Number(nr)))){
					td1.classList = 'colorTab2';
					k++;	
				}
				 else if (k == (Number(num) + Number(nr))){
					break;
				}
			}
		t1.appendChild(tr1);
